...
| `/ticket-watch` | **Daily ticket hygiene check** (Chatty/Joy Loyalty/Wishlist) — pull open tickets từ Avada Ticket API → flag `stale_no_update` (ticket thường ≥1 ngày không update hoặc vẫn pending chưa ai nhận, trừ ticket `[DFY]`/`[ONB]`), `dfy_stuck` (ticket DFY/ONB ≥2 ngày, task trong checklist chưa xong mà không update) → dedup theo ngày (ticket mới bị flag show chi tiết, cũ chỉ đếm) + **breakdown theo CS phụ trách** (`assigneeBreakdown`) + **VIP tier luôn show full** (`isVip` = plan khác free/basic) → DM Liz link/chat/nội dung/progress từng ticket qua Slack. Cron hàng ngày 10:00 sáng. Script: `skills/ticket-watch/scripts/fetch_stale.py`, gửi qua `skills/qa-weekly/scripts/send_dm.py` |
