...
| `/kb-sync` | **Đồng bộ FAQ tuần → KB CS v2** (Chatty/Ivy + Joy). Lấy file mined-FAQ mới nhất → so với KB live trên `cs2.avada.net` → diff (COVERED/OUTDATED/GAP/PARTIAL) → soạn patch file hiện tại → **Liz duyệt** → push (`POST /api/kb/file`, auto git commit) + reindex (`POST /api/kb/reindex`). Cron T3 11:30 (30p sau `/mine-chat-faqs` T3 11:00) chỉ chạy tới bước diff rồi DM Liz duyệt, KHÔNG tự push. Scripts: `skills/kb-sync/scripts/{prep,push_kb,kb_api}.py` |
