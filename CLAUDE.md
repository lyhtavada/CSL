...
| `/ticket-watch` | **Daily ticket hygiene check** (Chatty/Joy Loyalty/Wishlist) — pull open tickets từ Avada Ticket API → flag `stale_no_update` (≥2 ngày không update, trừ ticket `[DFY]`/`[ONB]` đã track riêng), `pending_unclaimed` (>24h chưa claim), `dfy_stuck` (checklist DFY dừng giữa chừng), `sla_breach` (P0/P1 quá SLA resolution theo `priority-matrix.md`) → dedup theo ngày (ticket mới bị flag show chi tiết, cũ chỉ đếm) → DM Liz link/chat/nội dung/progress từng ticket qua Slack. Cron hàng ngày 8:30 sáng. Script: `skills/ticket-watch/scripts/fetch_stale.py`, gửi qua `skills/qa-weekly/scripts/send_dm.py` |
