...
| `/cs-daily-brief` | **Báo cáo CS hàng ngày** (thay cho `/ticket-watch` cũ, đã gộp thêm) — cron 8:45 sáng, report cho **cửa sổ rolling 24h 08:30 hôm trước → 08:30 hôm nay** (VD chạy ngày 22 → báo cáo 21 08:30 → 22 08:30, canh theo giờ chạy để giảm độ trễ so với mốc 00:00 cũ). 4 phần: ① **Tổng quan conversation** Joy/Chatty/Wishlist (BigQuery `crisp_chats`, script `fetch_conversations.py`); ② **Checkin/checkout Team G2** late/miss (Admin API `/shifts`, script `fetch_checkin.py`); ③ **Chat AI đã tạo ticket** — theo từng app bot (Joyce/Joy, Ivy/Chatty, Wendy/Wishlist): số chat bot xử lý trong ngày (BigQuery `crisp_chats`, operator message `agentEmail IS NULL` + `userNickname` khớp tên bot) + list chat bot tạo ticket (Avada Ticket API, `members[].isCreate=true` với `memberId="ai-agent-2"`), format `[tên KH](link chat) — link ticket`; ④ **Ticket tạo cho Liz trong ngày** (Liz có trong `members[]`, script `fetch_liz_tickets.py`). Post vào kênh **#cs-2-daily** (`C0B8042TXQ9`) dưới tên Liz (live-fetch avatar/tên qua `users.info`), qua `skills/qa-weekly/scripts/send_dm.py` (đã sửa để nhận cả channel id `C...` chứ không chỉ user id `U...`). Scripts: `skills/cs-daily-brief/scripts/{fetch_conversations,fetch_checkin,fetch_ai_tickets,fetch_liz_tickets}.py` |
