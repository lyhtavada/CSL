...
| `/build-loyalty-program` | **AM/expansion** — thiết kế lại/thêm tier, referral, quest cho account **đang là khách hàng** (không phải prospect mới), xuất Excel 4 tab (Points/VIP/Referral/Milestones). Pull data thật qua avada-analytic MCP (`merchant_profile`, `cs_history`) + KB live thay vì file reference tĩnh. Output `reports/analysis/{account}-loyalty-program-{date}.xlsx`. Adapt từ skill sales cá nhân trong `~/Downloads/Joy Loyalty Sales Handover/06-skills/build-program-SKILL.md` (2026-08-25) |
