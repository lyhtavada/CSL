...
| `/count-reviews` | **Đếm review App Store ad-hoc** theo tháng/khoảng ngày cho Chatty/Joy — wrap `skills/cs-weekly/scripts/fetch_reviews.py` (scraper đã validate, dùng chung với `/cs-weekly`). KHÔNG dùng MCP `avada-analytic` (`cs_review_trend`/`cs_reviews`, số ra cao hơn thật) hay đếm tay trên listing (review bị edit nhảy vị trí sort → dễ sót/trùng). `--start/--end`, `--month YYYY-MM`, hoặc `--months 5,6,7,8 --year YYYY`. Wishlist chưa hỗ trợ (chưa xác nhận slug App Store) |
...
| `/store-research` | **Research 1 store để chuẩn bị call** (demo, onboarding, QBR, handover, save deal) — Liz đưa domain/email/tên brand → Betty pull hết: `merchant_profile`+`cs_history`+deal (MCP analytic), transcript Crisp đầy đủ (BigQuery), lịch call (Google Calendar), **config app thật đọc từ storefront** (Joy: earning/redeem/tier/referral + tự tính %hoàn và tổng điểm free không cần mua hàng), website merchant → viết 1 page **research + kịch bản call SPIN** theo template riêng từng app (`skills/store-research/templates/{joy,chatty}.md` — chung bộ section để so sánh được giữa các store, khác phần Trạng thái sản phẩm + checklist Gap + demo mapping; section không có data vẫn giữ heading và ghi lý do) → **push thẳng Notion** (không cần duyệt) vào `Joy Stores` (`38fb0da449f1808cbf1beb435fd6018c`) hoặc `Chatty Stores` (`3d3b0da449f1809abf8cea8e9f18d7d8`), page mới lên đầu + lưu bản md ở `reports/store-research/{app}/`. Script: `skills/store-research/scripts/fetch_store.py` (dùng lại `dfy-weekly-chatty/scripts/push_notion.py` để push). Khác `/chatty-icp-score` (chấm ICP) và `/account-research-note` (note lên deal timeline) |
| `/reply-reviews` | **Reply review app mobile Chatty** trên Apple App Store (iOS, Individual API Key của Liz) + Google Play (Android, service account `betty-play-reviews@lizs-502610`) — KHÁC `/count-reviews` (Shopify App Store). Fetch review chưa reply (bỏ reviewer nội bộ trong `config.json`) → soạn draft theo tone → **Liz duyệt** ("post 1,2" / "skip 3" / "sửa 1: ...") → mới post. Google Play API chỉ thấy review 7 ngày gần nhất, reply ≤350 ký tự. Cron T3+T6 17:00 gửi draft qua Telegram, **không bao giờ tự post**. Scripts: `skills/reply-reviews/scripts/reviews.py` + clients `tools/appstore/{asc,play}.py`, key trong `.secrets/` (gitignored) |
| `/share-secret` | Share password/API key/credential với team — tạo link view-once trên `secret.avada.net` thay vì paste Slack. Public (link+Google login) hoặc private (chỉ email chỉ định). Script: `skills/share-secret/scripts/share_secret.py`, token `SECRET_AVADA_TOKEN` trong `.env` |
| `/extract-wisdom` | Content-adaptive wisdom extraction từ video/podcast/article/text — tự phát hiện wisdom domain có trong content rồi build section riêng (không dùng header cố định IDEAS/QUOTES/HABITS/FACTS). 5 depth level (Instant/Fast/Basic/Full mặc định/Comprehensive). Giọng văn conversational, giữ nguyên spicy/contrarian take. Dùng khi Liz nói "extract wisdom", "tóm tắt video/podcast này", "key takeaways từ bài này". Adapt nguyên bản từ [Dylan-skill-library](https://github.com/DylanNgo1808/Dylan-skill-library/blob/main/skills/ExtractWisdom/SKILL.md), role-agnostic |
| `/screenshot` | **Chụp màn hình web → link ảnh public** (nhúng Notion/doc/Slack). Trang public chạy thẳng (`--launch`); trang cần đăng nhập (Shopify admin, app Chatty/Joy, Crisp, cs2, analytics) thì attach vào Chrome debug riêng của Betty (`tools/browser/start-chrome.sh`, profile `~/.betty-chrome` — Chrome 136+ chặn debug trên profile mặc định). Bấm/gõ trước khi chụp bằng `--do 'click:text=...'`, làm ảnh sạch bằng `--element`/`--hide`/`--mask`. **Luật: chỉ chụp dev/test store, ảnh upload là public theo link, luôn tự Read lại ảnh trước khi đưa link.** Tool: `tools/browser/{shot.mjs,upload.py,start-chrome.sh}` |
