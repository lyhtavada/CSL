...
| `/dfy-monthly` | **Monthly DFY report cho lãnh đạo** (Chatty only, no points) — pull DFY tickets tháng → tách 🔵 Inbound (không tag `proactive`) vs 🟢 Proactive (có tag) + adopt rate từng nhóm → auto insight (video→adopt, AI→adopt, Chatbox coverage, timing, review-yes, per-CS) → Notion sub-page (mới nhất lên đầu, parent `Chatty DFY Reports`) + Slack digest Block Kit **as Liz** vào channel CS (button "Xem full trên Notion", không tag ai). `--draft` (mặc định chạy tay) = push Notion + DM Liz preview; `--send`/cron = post thẳng channel. Cron T2 hàng tháng 10AM. Scripts: `skills/dfy-monthly/scripts/{fetch_dfy,build_report,push_notion,notify_slack}.py` |
...
| `/cs-daily-brief` | **Báo cáo CS hàng ngày** (thay cho `/ticket-watch` cũ, đã gộp thêm) — cron 9:30 sáng, report cho **ngày đầy đủ liền trước** (VD chạy ngày 22 → báo cáo ngày 21, 00:00–24:00 VN). 4 phần: ① **Tổng quan conversation** Joy/Chatty/Wishlist (BigQuery `crisp_chats`, script `fetch_conversations.py`); ② **Checkin/checkout Team G2** late/miss (Admin API `/shifts`, script `fetch_checkin.py`); ③ **Ticket watch** — snapshot hiện tại (không theo ngày report), pull tickets từ Avada Ticket API → bỏ qua ticket đã Done (`dueDateDone=true` hoặc `tsStatus=done`, kể cả khi `ticketStatus` chưa sync) → flag `stale_no_update` (ticket thường ≥1 ngày không update hoặc vẫn pending chưa ai nhận, trừ `[DFY]`/`[ONB]`), `dfy_stuck` (DFY/ONB ≥2 ngày, task checklist chưa xong không update) → dedup theo ngày (`state/seen.json`) + breakdown theo CS phụ trách (`assigneeBreakdown`) + VIP tier luôn show full (`isVip` = plan khác free/basic); ④ **Ticket tạo cho Liz trong ngày** (Liz có trong `members[]`, script `fetch_liz_tickets.py`). Post vào kênh **#cs-2-daily** (`C0B8042TXQ9`) dưới tên Liz (live-fetch avatar/tên qua `users.info`), qua `skills/qa-weekly/scripts/send_dm.py` (đã sửa để nhận cả channel id `C...` chứ không chỉ user id `U...`). Scripts: `skills/cs-daily-brief/scripts/{fetch_conversations,fetch_checkin,fetch_stale,fetch_liz_tickets}.py` |
