...
| `/cs-daily-brief` | **Báo cáo CS hàng ngày** (thay cho `/ticket-watch` cũ, đã gộp thêm) — cron 10:00 sáng, report cho **ngày đầy đủ liền trước** (VD chạy ngày 22 → báo cáo ngày 21, 00:00–24:00 VN). 3 phần: ① **Tổng quan conversation** Joy/Chatty/Wishlist (BigQuery `crisp_chats`, script `fetch_conversations.py`); ② **Checkin/checkout Team G2** late/miss (Admin API `/shifts`, script `fetch_checkin.py`); ③ **Ticket watch** — snapshot hiện tại (không theo ngày report), pull tickets từ Avada Ticket API → flag `stale_no_update` (ticket thường ≥1 ngày không update hoặc vẫn pending chưa ai nhận, trừ `[DFY]`/`[ONB]`), `dfy_stuck` (DFY/ONB ≥2 ngày, task checklist chưa xong không update) → dedup theo ngày (`state/seen.json`) + breakdown theo CS phụ trách (`assigneeBreakdown`) + VIP tier luôn show full (`isVip` = plan khác free/basic). Post vào kênh **#cs-2-daily** (`C0B8042TXQ9`) dưới tên Liz (live-fetch avatar/tên qua `users.info`), qua `skills/qa-weekly/scripts/send_dm.py` (đã sửa để nhận cả channel id `C...` chứ không chỉ user id `U...`). Scripts: `skills/cs-daily-brief/scripts/{fetch_conversations,fetch_checkin,fetch_stale}.py` |
