"""
Convert raw helpcenter articles into Q&A training data for Chatty AI copilot.
...
def clean_artifacts(text: str) -> str:
    ...
    # Remove arrow-pointer from link text
    text = re.sub(r"arrow-pointer", "", text)
    # Remove chevron artifacts in link text
    text = re.sub(r"chevron-(left|right)", "", text)
    # Remove icon artifacts on their own line
    text = re.sub(
        r"^(circle-info|circle-exclamation|circle-check|chevron-right|hashtag|check|envelope)\s*$",
        "", text, flags=re.MULTILINE
    )
    # Remove "- check\n\n  Text" patterns (GitBook checklist items) → "- Text"
    text = re.sub(r"- check\n\n\s{2}", "- ", text)
    return text
...
def clean_markdown(text: str) -> str:
    ...
    # Remove standalone Previous/Next links
    ...
    # Remove "Who can use this feature?" blocks
    text = re.sub(r"^\*{0,2}Who can use (this feature|Chatty)\?\*{0,2}\s*\n+- .+\n*", "", text, flags=re.MULTILINE)
    # Remove leading "# Title\n\n" if the raw file starts with an H1
    text = re.sub(r"^# .+\n+", "", text, count=1)
    # Remove standalone numbers used as step indicators (GitBook stepper)
    text = re.sub(r"^\d+\s*$", "", text, flags=re.MULTILINE)
    # Remove remaining inline icon artifacts
    text = re.sub(r"\n\s*circle-info\s*\n", "\n\n", text)
    text = re.sub(r"\n\s*circle-exclamation\s*\n", "\n\n", text)
    text = re.sub(r"\n\s*circle-check\s*\n", "\n\n", text)
    # Remove /cdn-cgi/ email protection links
    text = re.sub(r"\[email envelope\]\(/cdn-cgi/[^)]*\)", "email", text)
...
def extract_title_from_file(text: str, filename: str) -> str:
    """Extract a clean title. Prefer H1 from content, then first H3, fallback to filename slug."""
    # Try H1 first
    ...
    # Try first H3 (common in GitBook pages without H1)
    m = re.search(r"^### (.+)$", text, re.MULTILINE)
    if m:
        title = m.group(1).strip()
        title = clean_artifacts(title).strip()
        # Remove leading question phrases for cleaner topic names
        title = re.sub(r"^(What (is|are)|How to|How do I) ", "", title, flags=re.I).rstrip("?")
        if len(title) > 2 and not title.startswith("#"):
            return title
...
def extract_faq_section(text: str) -> list:
...
def slug_to_readable(slug: str) -> str:
    """Convert URL slug to readable text: 'ai-skills' -> 'AI Skills'."""
    result = slug.replace("-", " ").replace(".", " ")
    result = result.title()
    for term, replacement in [
        ("Pos", "POS"), ("Vip", "VIP"), ("Sms", "SMS"),
        ("Api", "API"), ("Faq", "FAQ"), ("Csv", "CSV"),
        ("Ai", "AI"), ("Qr", "QR"), ("Faqs", "FAQs"),
        ("Url", "URL"), ("Whatsapp", "WhatsApp"),
    ]:
...
def generate_questions(title: str, category: str, subcategory: str, content: str) -> list:
    """Generate likely merchant questions based on article title and content."""
    questions = []
    content_lower = content.lower()
    cat_lower = category.lower()

    is_howto = bool(re.search(r"how to (set up|integrate|configure|use|enable|create|add|connect)", content, re.I))
    is_integration = "integrat" in cat_lower or "integrat" in content_lower[:300]
    is_troubleshoot = bool(re.search(r"(troubleshoot|common (issue|reason|problem)|not (work|show|receiv))", content, re.I))

    # Base question
    questions.append(f"What is {title} in Chatty?")
    ...
    if is_integration:
        questions.append(f"How do I connect {title} with Chatty?")

    # Troubleshooting
    if is_troubleshoot:
        questions.append(f"Why is {title} not working?")

    # Category-contextual (avoid redundant questions when title ~ category)
    title_lower = title.lower()
    if "live chat" in cat_lower and "live chat" not in title_lower:
        questions.append(f"How do I use {title} in live chat?")
    elif "ai" in cat_lower and title_lower not in ("ai", "ai assistant"):
        questions.append(f"How does {title} work with AI assistant?")
    elif "chatbox" in cat_lower:
        questions.append(f"How do I customize {title} on my store?")
    elif "build faq" in cat_lower or "faq" in cat_lower:
        questions.append(f"How do I manage {title} for my store?")
    elif "mobile" in cat_lower or "web app" in cat_lower:
        questions.append(f"How do I use {title} on mobile/web?")
    elif "other" in cat_lower:
        questions.append(f"Where can I find {title} settings?")
    elif "product roadmap" in cat_lower:
        questions.append(f"What are the latest updates for {title}?")
    elif "privacy" in cat_lower:
        questions.append(f"What is Chatty's policy on {title}?")
...
def process_article(raw_path: str, rel_path: str):
