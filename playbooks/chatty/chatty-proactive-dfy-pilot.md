# Chatty Proactive DFY — Pilot SOP

**Mục đích:** Chuyển DFY từ **thụ động** (chờ KH vào từ chat rồi offer) sang **chủ động** — CS/TS tự lấy danh sách KH mới cài Pro/Plus, chủ động offer DFY và follow-up qua các ca.

**Người chạy:** CS/TS (giai đoạn pilot bắt đầu với 1 CS đang training sub-lead, 4 ca/tuần)
...
Sau audit, CS/TS viết **một email cá nhân hoá** cho store đó — nêu cụ thể cái thấy được từ audit + đề xuất setup, rồi gửi luôn. Đây là điểm khác DFY thụ động: chủ động đến tận nơi, không pop-up chờ KH bấm.
...
4. **CTA rõ một hành động** — reply để CS bắt đầu, hoặc đề xuất khung giờ.
...
**CS/TS tự tạo DFY ticket cho store đó** (KHÔNG dùng Google Sheet). Mỗi store = 1 ticket. Tạo ngay khi gửi email (để track cả KH chưa reply), ghi nội dung email + kết quả audit vào ticket.
- `appName` = Chatty, `appPlan`, `domain` (store đang làm)
- Member = CS/TS đang làm store đó
...
| 5 | Brief CS/TS: tệp = bảng analytics, làm theo SOP này + flow gốc | Liz |
