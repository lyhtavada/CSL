...
FILL_NOTE = "Fill in the Value column (red cells)."
...
def build_setup(d):
    ...
    t.colheader(["Item", "Value", "Notes"])
    t.data(["Program name", d.get("program_name", "")], [1])
    t.data(["Point currency name", d.get("point_currency_name", ""), "branded currency = feels owned"], [1])
    t.data(["Point value", d.get("point_value", ""), "~5% rebate rate"], [1])
    t.data(["Base earn rate", d.get("base_earn_rate", ""), "tier multipliers added in VIP tab"], [1])
    t.data(["Point expiry", d.get("point_expiry", ""), "win-back email before expiry"], [1])
    t.data(["Coupon expiry", d.get("coupon_expiry", ""), "gives customers time to use"], [1])
    t.blank()
    t.section("APP INTEGRATIONS")
    t.colheader(["Item", "Value", "Notes"])
    t.data(["Email / SMS", d.get("integ_email_sms", ""), "reward + win-back emails"], [1])
    t.data(["Reviews", d.get("integ_reviews", ""), "links to Earning tab"], [1])
    t.data(["Shopify POS", d.get("integ_pos", ""), "omnichannel"], [1])
    t.data(["Shopify Flow", d.get("integ_flow", ""), "used in Earning tab"], [1])
    t.data(["Subscriptions", d.get("integ_subscriptions", ""), "reward subscribers"], [1])
    t.plus()
    t.blank()
    t.blank()
    t.blank()
    ...
    t.colheader(["Item", "Value", "Notes"])
    t.data(["Data to import", d.get("migration_data", ""), "export CSV from old app"], [1])
    t.data(["Import date", d.get("migration_date", ""), "freeze old app after export"], [1])
    return t
...
def build_earning(d):
    ...
    t.blank()
    t.colheader(["Rule", "Action (Joy)", "Points Earned", "Note"])
    t.data(["Purchase Reward", "Place Order", d.get("earn_purchase", "")], [2])
    t.data(["Welcome Bonus", "Sign-Up", d.get("earn_welcome", "")], [2])
    t.data(["Newsletter Sign-Up", "Newsletter Sign-Up", d.get("earn_newsletter", "")], [2])
    t.data(["Birthday Gift", "Birthday Reward", d.get("earn_birthday", "")], [2])
    t.data(["Product Review", "Write Review", d.get("earn_review", "")], [2])
    t.data(["Photo/Video Review", "Write Review (media)", d.get("earn_review_media", "")], [2])
    t.data(["Google Review", "Google Reviews", d.get("earn_google_review", "")], [2])
    t.data(["Follow Instagram", "Social Activity", d.get("earn_ig", "")], [2])
    t.data(["Follow TikTok", "Social Activity", d.get("earn_tiktok", "")], [2])
    t.data(["Social Share", "Custom (Shopify Flow)", d.get("earn_social_share", "")], [2])
    t.plus()
    t.blank()
    t.blank()
    t.blank()
    ...
    t.colheader(["Tier", "Reward", "Value", "Note"])
    t.data(["Tier 1 (base)", "Points", d.get("earn_birthday_tier1", "")], [2])
    t.data(["Tier 2", "Points + gift", d.get("earn_birthday_tier2", "")], [2])
    t.data(["Tier 3", "Points + gift + perk", d.get("earn_birthday_tier3", "")], [2])
    t.plus()
    return t
...
def build_redemption(d):
    ...
    t.colheader(["General Rule", "Value"])
    t.data(["Min points to redeem", d.get("redeem_min_points", "")], [1])
    t.data(["Coupon expiration", d.get("redeem_coupon_expiry", "")], [1])
    t.blank()
    t.section("REWARDS — DISCOUNT (AMOUNT / PERCENTAGE)")
    t.colheader(["Reward", "Type", "Cost (Points)", "Min purchase amount", "Discount combination", "Note"])
    t.data(["Fixed amount off", "Amount off", d.get("discount_fixed1_cost", ""), d.get("discount_fixed1_min", ""), d.get("discount_fixed1_combo", "")], [2, 3, 4])
    t.data(["Fixed amount off", "Amount off", d.get("discount_fixed2_cost", ""), d.get("discount_fixed2_min", ""), d.get("discount_fixed2_combo", "")], [2, 3, 4])
    t.data(["Percentage off", "% off", d.get("discount_pct_cost", ""), d.get("discount_pct_min", ""), d.get("discount_pct_combo", "")], [2, 3, 4])
    t.plus()
    t.blank()
    t.blank()
    t.blank()
    ...
    t.colheader(["Reward", "Product (names or links)", "Cost (Points)", "Min purchase amount", "Discount combination", "Note"])
    t.data(["Free product", d.get("gift_product", ""), d.get("gift_cost", ""), d.get("gift_min", ""), d.get("gift_combo", "")], [1, 2, 3, 4])
    t.plus()
    t.blank()
    t.blank()
    t.blank()
    ...
    t.colheader(["Reward", "Type", "Cost (Points)", "Min purchase amount", "Discount combination", "Note"])
    t.data(["Free shipping", "Free ship", d.get("shipping_cost", ""), d.get("shipping_min", ""), d.get("shipping_combo", "")], [2, 3, 4])
    t.plus()
    return t
...
def build_vip(d):
    ...
    t.colheader(["Tier Config", "Value"])
    t.data(["Tier calculated by", d.get("vip_calc_by", "")], [1])
    t.data(["Evaluation window", d.get("vip_eval_window", "")], [1])
    t.data(["Point earn multiplied by tier?", d.get("vip_multiplier_applies", "")], [1])
    t.data(["Re-evaluation cycle", d.get("vip_reeval_cycle", "")], [1])
    t.blank()
    t.section("TIERS")
    t.colheader(["Tier", "Condition to Reach", "Earn Multiplier (purchase only - optional) ", "Entry rewards", "Entry reward combo", "Perk", "Perk combo"])
    t.data(["Tier 1 (e.g. Silver / Sipper)", d.get("tier1_condition", ""), d.get("tier1_multiplier", ""), d.get("tier1_entry", ""), d.get("tier1_entry_combo", ""), d.get("tier1_perk", ""), d.get("tier1_perk_combo", "")], [1, 2, 3, 4, 5, 6])
    t.data(["Tier 2 (e.g. Gold / Steeper)", d.get("tier2_condition", ""), d.get("tier2_multiplier", ""), d.get("tier2_entry", ""), d.get("tier2_entry_combo", ""), d.get("tier2_perk", ""), d.get("tier2_perk_combo", "")], [1, 2, 3, 4, 5, 6])
    t.data(["Tier 3 (e.g. Platinum / Master)", d.get("tier3_condition", ""), d.get("tier3_multiplier", ""), d.get("tier3_entry", ""), d.get("tier3_entry_combo", ""), d.get("tier3_perk", ""), d.get("tier3_perk_combo", "")], [1, 2, 3, 4, 5, 6])
    t.plus()
    return t
...
def build_referral(d):
    ...
    t.title("Referral")
    t.blank()
    t.colheader(["Item", "Value"])
    t.data(["Referrer gets", d.get("referrer_gets", "")], [1])
    t.data(["Referred friend gets", d.get("referred_gets", "")], [1])
    t.data(["Condition", d.get("referral_condition", "")], [1])
    t.data(["Referral points multiplied by tier?", d.get("referral_by_tier", "")], [1])
    t.data(["Referral banner (widget)", d.get("referral_banner", "")], [1])
    t.plus()
    t.blank()
    t.blank()
    t.blank()
    ...
    t.colheader(["Tier", "Referrer gets"])
    t.data(["Tier 1 (base)", d.get("referral_tier1", "")], [1])
    t.data(["Tier 2", d.get("referral_tier2", "")], [1])
    t.data(["Tier 3", d.get("referral_tier3", "")], [1])
    return t
...
def build_milestone(d):
    ...
    t.blank()
    t.colheader(["Milestone", "Type", "Target", "Reward", "Note"])
    t.data(["First order", "Order count", d.get("milestone_first_order_target", ""), d.get("milestone_first_order_reward", "")], [2, 3])
    t.data(["Loyal customer", "Order count", d.get("milestone_loyal_target", ""), d.get("milestone_loyal_reward", "")], [2, 3])
    t.data(["Big spender", "Total spend", d.get("milestone_big_spender_target", ""), d.get("milestone_big_spender_reward", "")], [2, 3])
    t.data(["Anniversary", "Account age", d.get("milestone_anniversary_target", ""), d.get("milestone_anniversary_reward", "")], [2, 3])
    t.plus()
    return t
...
def build_launch(d):
    ...
    t.colheader(["Item", "Value", "Notes"])
    t.data(["Widget installed & on-brand", d.get("launch_widget", ""), "check on desktop + mobile"], [1])
    t.data(["Program name & T&C page live", d.get("launch_terms_page", ""), "link in footer"], [1])
    t.data(["Email/SMS templates connected", d.get("launch_email_templates", ""), "via Klaviyo/Omnisend"], [1])
    ...
    t.colheader(["Item", "Value", "Notes"])
    t.data(["Soft launch group", d.get("bootstrap_soft_launch_group", ""), "test before public launch"], [1])
    t.data(["Sign-up bonus for existing customers", d.get("bootstrap_signup_bonus", ""), "rewards signing up during bootstrap window"], [1])
    t.data(["Retroactive points for past purchases", d.get("bootstrap_retroactive_points", ""), "optional, boosts perceived value at day 1"], [1])
    t.data(["Migration import (if migrating)", d.get("bootstrap_migration", ""), "see Setup tab → Migration/Import"], [1])
    t.data(["Launch promo", d.get("bootstrap_promo", ""), "drives first-week engagement"], [1])
    t.plus()
    t.blank()
    t.blank()
    ...
    t.data(["Announcement channels", d.get("public_announcement_channels", "")], [1])
    ...
    t.colheader(["Item", "Value", "Notes"])
    t.data(["Redemption rate target", d.get("monitor_redemption_target", ""), "too low = low perceived value"], [1])
    t.data(["Point liability check-in", d.get("monitor_liability_checkin", ""), "catch runaway earn rate early"], [1])
    t.data(["Follow-up win-back email", d.get("monitor_winback_email", "")], [1])
    t.plus()
    return t
