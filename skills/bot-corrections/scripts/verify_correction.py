#!/usr/bin/env python3
"""
verify_correction.py — mark a bot correction as verified-correct when the
TRIAGE finds the CS's correction was itself wrong / unnecessary (bot's
original_response was actually fine) — so it stops looking like an open bot
error in QA reports.
...
2026-08-21). The row schema does expose `status` ("draft" seen so far),
...
this is a best guess. Defaults to --dry-run (prints the payload, sends
nothing). Pass --live to actually PUT. On the FIRST live run, check the
result against the cs2.avada.net dashboard by hand before trusting this
unattended in cron.

Usage:
  python3 verify_correction.py --id 1655 --note "CS correction was inaccurate; bot's original reply was correct" [--live]
"""
import argparse
import json
import os
...
def main():
    ...
    body = {"status": "verified", "verified_by": "betty", "reviewed_note": args.note}
