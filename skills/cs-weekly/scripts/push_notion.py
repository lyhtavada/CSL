...
def table_block(rows):
    ...
    the top-level page children list. A table with >100 rows must ship its first
    ~100 rows inline and the rest via a separate append to the table block's own
    id once it exists. Returns (block_with_first_100_rows, overflow_row_blocks)."""
