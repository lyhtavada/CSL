---
name: extract-wisdom
description: "Content-adaptive wisdom extraction that reads the content first, detects what wisdom domains are actually present, then builds custom sections around what it finds — instead of forcing static headers every time. A security talk gets 'Threat Model Insights' and 'Defense Strategies'; a business podcast gets 'Contrarian Business Takes' and 'Money Philosophy'. Five depth levels: Instant (1 section), Fast (3 sections), Basic (3+takeaway), Full (5-12 sections, default), Comprehensive (10-15+themes). Voice follows a conversational style — bullets read like telling a friend what you just watched, not a press release. Output always includes dynamic sections, One-Sentence Takeaway, 'If You Only Have 2 Minutes', and References. Spicy/contrarian takes are mandatory inclusions, never softened. USE WHEN extract wisdom, analyze video, analyze podcast, extract insights, extract from YouTube, key takeaways, what's interesting in this, process this content, summarize interview, analyze article, extract from blog post, what are the main takeaways, distill this content."
---

# ExtractWisdom — Dynamic Content Extraction

Instead of static sections (IDEAS, QUOTES, HABITS, FACTS — the same headers every time), this skill detects what wisdom domains actually exist in the content and builds custom sections around them.
...
- When standard summarization misses the gems
...
| Source | Method |
|--------|--------|
| YouTube URL | If you have a transcript CLI available (e.g. `fabric -y "URL"`, `yt-dlp` + captions), use it. Otherwise fetch the video page, ask the user to paste the transcript, or use whatever transcript tool you have. |
| Article URL | Fetch the page content directly. |
| File path | Read the file directly. |
| Pasted text | Use directly. |

Save large transcripts to a working file before extracting. Long content (a 3-hour podcast, say) may need chunking — don't try to extract wisdom from it in one pass.

## The Core Idea

Read the content first. Figure out what's actually in there. Build sections around what you find.
...
The bullets should sound like someone telling a friend about it over coffee. Not compressed info nuggets. Not clever one-liners. Actual spoken observations.
...
**Level 3 (YES — conversational, a real voice):**
...
- **YouTube extraction needs a transcript first.** Use whatever transcript tool you have available (a CLI, WebFetch on the video page, or asking the user to paste it) — the extraction quality depends entirely on transcript quality, not on which tool got it.
...
```
User: "extract wisdom from this interview"
→ Get the transcript (transcript CLI, or WebFetch, or pasted text)
...
Adapted from [DylanNgo1808/Dylan-skill-library](https://github.com/DylanNgo1808/Dylan-skill-library/blob/main/skills/ExtractWisdom/SKILL.md) — role-agnostic, no changes needed for this workspace.
